...
public final class HostAndPort implements Serializable {
    ...
    public HostAndPort(final String host, final int port) {
        final Pair<String, Integer> translated = addressTranslators.
                stream().
                filter(p -> p.getLeft().getLeft().equals(host) && p.getLeft().getRight().equals(port)).
                findAny().
                map(Pair::getRight).
                orElse(null);

        this.host = translated != null ? translated.getLeft() : host;
